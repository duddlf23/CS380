...
static void drawStuff() {
  ...
  RigTForm sphRbt;
  bool cnt = false;
  if (index_mani == 0 && index_w_s == 0 && index_view==0){
	  sphRbt = g_objectRbt[2];
	  cnt = true;
  }
  else if (index_mani == 1 && index_view != 1){
	  sphRbt = g_objectRbt[0];
	  cnt = true;
  }
  else if (index_mani == 2 && index_view != 2){
	  sphRbt = g_objectRbt[1];
	  cnt = true;

  }
  if (cnt){
	  MVM = invEyeRbt * sphRbt;
	  ...
	  NMVM = normalMatrix(rigTFormToMatrix(MVM) * sc);
...
static void motion(const int x, const int y) {
  ...
  RigTForm A;
  if (index_mani == 0 && index_view == 0 && index_w_s == 0){
	  A = linFact(eyeRbt);
  }
  else if (index_mani == 0 && index_view == 0) {
	  A = transFact(g_skyRbt) * linFact(eyeRbt);

  }
  else if (index_mani == 1){
	  A = transFact(g_objectRbt[0]) * linFact(eyeRbt);

  }
  else if (index_mani == 2){
	  A = transFact(g_objectRbt[1]) * linFact(eyeRbt);
  ...
  if (g_mouseClickDown) {
	  if (index_mani == 0 && index_view == 0){
		  if (index_w_s == 0) g_skyRbt = A*inv(m)*invA*g_skyRbt;
		  else g_skyRbt = g_skyRbt * (transFact(m)*inv(linFact(m)));
	  }
	  else if (index_mani == 1){
		  if (index_view == 1) g_objectRbt[0] = g_objectRbt[0] * (transFact(m)*inv(linFact(m)));
		  else g_objectRbt[0] = A*m*invA*g_objectRbt[0];
	  }
	  else if (index_mani == 2){
		  if (index_view == 2) g_objectRbt[1] = g_objectRbt[1] * (transFact(m)*inv(linFact(m)));
		  else g_objectRbt[1] = A*m*invA*g_objectRbt[1];
