...
static void motion(const int x, const int y) {
  ...
  if (g_mouseClickDown) {
	  if (index_mani == 0){
		  ...
		  //g_skyRbt*= m;
		  if (index_w_s == 0) g_skyRbt = inv(m)*g_skyRbt;
		  else g_skyRbt *= transFact(m)*inv(linFact(m));
	  }
	  else if (index_mani == 1){
		  if (index_view == 1) g_objectRbt[0] *= transFact(m)*inv(linFact(m)); 
		  else g_objectRbt[0] = A*m*invA*g_objectRbt[0];
	  }
	  else{
		  if (index_view == 2) g_objectRbt[1] *= transFact(m)*inv(linFact(m));
